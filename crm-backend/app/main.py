...
async def lifespan(app: FastAPI):
    # Dev convenience: create tables on startup. (Production would use Alembic
    # migrations — a conscious scope cut.)
    Base.metadata.create_all(bind=engine)
    _seed_if_empty()
    # Standing background worker for event-triggered automations.
...
app = FastAPI(title="Brewhaus CRM", version="0.1.0", lifespan=lifespan)
